	...
	b["RequestItems"] = func() msi {
		...
		for table, itemActions := range tableItems {
			out[table.Name] = func() interface{} {
				...
				// here breaks order of array....
				// For now, we iterate over sorted key... for safe-testing
				keys := []string{}
...
func (q *Query) AddCreateRequestTable(description TableDescription) {
	b := q.buffer
...
func (q *Query) AddDeleteRequestTable(description TableDescription) {
	b := q.buffer
	b["TableName"] = description.TableName
}
